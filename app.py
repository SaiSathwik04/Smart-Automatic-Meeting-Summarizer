...
def summarize_text(text):
    ...
    inputs = tokenizer(
        text, return_tensors="pt", max_length=1024, truncation=True, padding=True
    )
...
st.title("Smart Automatic Meeting Summarization Tool")
...
if audio_file is not None:
    ...
    if not st.session_state.transcription_done:
        ...
        if transcribed_text:
            ...
            st.success(
                "Transcription completed successfully. Now performing summarization..."
            )
        else:
            st.error(
                "No transcription found. Please try uploading a different audio file."
            )
    ...
    if st.session_state.summary_done:
        ...
        precision, recall, f1, accuracy = evaluate_summary(
            st.session_state.transcribed_text, st.session_state.summary
        )
        similarity = validate_transcription(
            st.session_state.transcribed_text, st.session_state.summary
        )
